"""
LoRA Fine-tuning for Disaster Tweets Classification
Supports 1B-2B parameter models with efficient LoRA training
...
Usage:
    python src/train_lora.py --model qwen2.5-1.5b --epochs 5 --batch_size 16
"""
...
def get_lora_config(args):
    ...
    if args.lora_target_modules:
        ...
        # Default: attention layers only (most efficient)
...
def main(args):
    ...
    # Load base model and apply LoRA (or resume from checkpoint)
    if args.resume_from_checkpoint:
        ...
        print("Successfully loaded LoRA adapters from checkpoint!")
    ...
    output_dir = os.path.join(args.output_dir, f"{args.model}_lora_{timestamp}")
    ...
    for epoch in range(args.epochs):
        ...
        if val_f1 > best_f1:
            best_f1 = val_f1
            best_epoch = epoch + 1
            print(f"\n🎉 New best F1 score! Saving LoRA adapters...")

            # Save LoRA adapters
            model.save_pretrained(os.path.join(output_dir, 'best_lora_adapters'))
            tokenizer.save_pretrained(os.path.join(output_dir, 'best_lora_adapters'))
    ...
    print(f"LoRA adapters saved to: {output_dir}/best_lora_adapters")
    print("="*60)
...
def parse_args():
    parser = argparse.ArgumentParser(description='LoRA Fine-tuning for Disaster Tweets')
    ...
    parser.add_argument('--lora_target_modules', type=str, default='',
                        help='Comma-separated target modules (empty for default)')
    ...
    parser.add_argument('--learning_rate', type=float, default=1e-4,
                        help='Learning rate (higher than full fine-tuning)')
    ...
    parser.add_argument('--resume_from_checkpoint', type=str, default='',
                        help='Path to LoRA adapter checkpoint to resume training from')
